...
7. AI agent compliance check is mandatory: before finalizing changes, verify the implementation aligns with `.github/copilot-instructions.md` and relevant `.github/skills/` guidance. **When a compliance check is run, also audit dependencies and security**: (a) verify all packages in `Package.swift` / `Package.resolved` are from known, legitimate publishers, (b) check for outdated or unmaintained dependencies (last commit > 12 months), (c) scan for known vulnerabilities (CVEs, security advisories) against current dependency versions, (d) confirm no secrets, API keys, or credentials are hardcoded in source or config files, (e) validate that all network calls use HTTPS, timeouts, and input validation, (f) ensure error messages do not leak internal details (stack traces, raw error descriptions, file paths).
...
|-------|---|---|
| **Models** | Pure data types (`struct`), `Codable` conformance, `Identifiable`. **Zero framework dependencies.** | `Models/RssFeedModel.swift`, `Models/FeedItem.swift`, `Models/FeedRecord.swift` |
| **Services** | Network requests (`URLSession`), XML parsing (`XMLParser`), SQLite persistence. Pure I/O layer. | `Services/FeedService.swift`, `Services/RSSXMLParser.swift`, `Services/SQLiteFeedStore.swift`, `Services/OpenGraphService.swift`, `Services/DefaultFeeds.swift` |
| **ViewModels** | `ObservableObject` with `@Published` state. Orchestrates services, manages loading/error state. **Marked `@MainActor`.** | `ViewModels/FeedViewModel.swift`, `ViewModels/ImageResolver.swift` |
| **Views** | Declarative SwiftUI templates. Reads ViewModel state. No business logic. | `Views/ContentView.swift`, `Views/DashboardView.swift`, `Views/ManageFeedsView.swift`, `Views/SettingsView.swift` |
| **Utils** | Pure helper functions (date formatting, string manipulation). No dependencies. | `Utils/Helpers.swift` |
| **Resources** | Static assets bundled with the app (icons, AI model registry). | `Resources/Assets.xcassets/`, `Resources/ai_models.json` |
